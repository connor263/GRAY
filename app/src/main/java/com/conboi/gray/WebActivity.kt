...
class WebActivity : AppCompatActivity() {
    ...
    var fileData: ValueCallback<Uri>? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val link = cacheLinkRepositoryImpl.getLink()

        webView = findViewById<WebView>(R.id.webView)
        initWeb(webView, link)
    }

    private fun initWeb(webView: WebView, link: String) = with(webView) {
        setSettings(this)
        webViewClient = WebClients().viewClient
        webChromeClient = WebClients().chromeClient
        loadUrl(link)
    }
    ...
    override fun onBackPressed() {
        ...
        webView.apply {
            when {
                isFocused && canGoBack() -> goBack()
                else -> println(packageName)
            }
        }
    }
}
